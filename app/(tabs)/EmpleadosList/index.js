...
export default function EmpleadosList() {
  ...
  const handleEmployeePress = (employee) => {
    ...
    Alert.alert(
      ...
      [
        {
          text: 'Edit',
          onPress: () => router.push(`/(tabs)/EmpleadosList/EmpleadoEditar?id=${employee.id}`)
        },
        {
          text: 'Delete',
  ...
  const confirmDelete = (employee) => {
    Alert.alert(
      'Delete Employee',
      `Are you sure you want to delete ${employee.firstName} ${employee.lastName1}?`,
      [
      ...
      <View className="flex-row items-center">
        <View className="w-12 h-12 bg-primary rounded-full items-center justify-center mr-4">
          <Text className="text-primary-foreground font-semibold text-lg">
            {item.firstName.charAt(0)}{item.lastName1.charAt(0)}
          </Text>
        </View>
      ...
      <View className="flex-1 p-4">
        <View className="flex-row justify-between items-center mb-6">
          ...
          {user?.role === 'admin' && (
            ...
            >
              <Text className="text-primary-foreground font-medium">Add Employee</Text>
            </TouchableOpacity>
          )}
        </View>
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <View className="flex-1 justify-center items-center py-12">
              ...
              <Text className="text-muted-foreground text-lg mt-4">No employees found</Text>
            </View>
          }
        />
      </View>
